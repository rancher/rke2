...
# Build environment
FROM rancher/hardened-build-base:v1.20.6b2 AS build
...
RUN curl -sL https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh | sh -s v1.51.2
RUN set -x \
    && apk --no-cache add \
    libarchive-tools \
    zstd \
...
RUN CHART_VERSION="v3.25.1-build2023051203"   CHART_FILE=/charts/rke2-canal.yaml          CHART_BOOTSTRAP=true   /charts/build-chart.sh
...
FROM rancher/hardened-kubernetes:v1.24.16-rke2r1-build20230719 AS kubernetes
