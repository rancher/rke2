...
RUN CHART_VERSION="v3.20.1-build2021111904"   CHART_FILE=/charts/rke2-canal.yaml          CHART_BOOTSTRAP=true   /charts/build-chart.sh
...
RUN CHART_VERSION="v1.21.7-rke2r1-build2021111901" \
...
RUN CHART_VERSION="0.1.300"                   CHART_FILE=/charts/harvester-cloud-provider.yaml CHART_BOOTSTRAP=true /charts/build-chart.sh
RUN CHART_VERSION="0.1.400"                   CHART_FILE=/charts/harvester-csi-driver.yaml     CHART_BOOTSTRAP=true /charts/build-chart.sh
...
FROM rancher/k3s:v1.21.7-k3s1 AS k3s
FROM rancher/hardened-kubernetes:v1.21.7-rke2r1-build20211119 AS kubernetes
FROM rancher/hardened-containerd:v1.4.12-k3s1-build20211118 AS containerd
FROM rancher/hardened-crictl:v1.21.0-build20211118 AS crictl
FROM rancher/hardened-runc:v1.0.2-build20211118 AS runc
