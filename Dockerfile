...
# Build environment
FROM rancher/hardened-build-base:v1.23.10b1 AS build
...
ENV DAPPER_ENV GODEBUG GOCOVER REPO TAG GITHUB_ACTION_TAG PAT_USERNAME PAT_TOKEN KUBERNETES_VERSION DOCKER_BUILDKIT DRONE_BUILD_EVENT IMAGE_NAME AWS_SECRET_ACCESS_KEY AWS_ACCESS_KEY_ID ENABLE_REGISTRY DOCKER_USERNAME DOCKER_PASSWORD GH_TOKEN REGISTRY
ARG DAPPER_HOST_ARCH
ENV ARCH $DAPPER_HOST_ARCH
...
FROM rancher/hardened-kubernetes:v1.30.14-rke2r1-build20250618 AS kubernetes
FROM rancher/hardened-containerd:v1.7.27-k3s1-build20250613 AS containerd
FROM rancher/hardened-crictl:v1.30.1-build20250612 AS crictl
FROM rancher/hardened-runc:v1.2.6-build20250612 AS runc
