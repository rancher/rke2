...
func (k *KubernetesConfig) pod(args *podtemplate.Args) (*corev1.Pod, error) {
	...
	for _, dir := range secretDirs {
		prefix := filepath.Join(k.DataDir, "server", dir) + string(filepath.Separator)
			...
			volume := corev1.Volume{
				...
				VolumeSource: corev1.VolumeSource{
					Secret: &corev1.SecretVolumeSource{
						SecretName: fmt.Sprintf("%s-server-%s", k.Name, pg.base),
					},
				},
			}
...
func (k *KubernetesConfig) extractClusterSecrets(ctx context.Context, dirs ...string) error {
	secrets := k.client.CoreV1().Secrets(k.namespace)
	for _, dir := range dirs {
		secret, err := secrets.Get(ctx, k.Name+"-server-"+dir, metav1.GetOptions{})
		if err != nil {
		...
		if err := k.dirFromBytes(filepath.Join(k.DataDir, "server", dir), secret.Data); err != nil {
			return err
		}
	}
	return nil
...
func (k *KubernetesConfig) etcdConfigSecret(e *executor.ETCDConfig, extraArgs []string, replicas int32) (*corev1.Secret, error) {
	memberList := []string{}
	for i := int32(0); i < replicas; i++ {
		hostname := fmt.Sprintf("%s-etcd-%d.%s-etcd", k.Name, i, k.Name)
		if k.Domain != "" {
			hostname = fmt.Sprintf("%s.%s.svc.%s", hostname, k.namespace, k.Domain)
		}

		memberList = append(memberList, fmt.Sprintf("%s-etcd-%d=https://%s:2380", k.Name, i, hostname))
	}
	...
	for i := int32(0); i < replicas; i++ {
		...
		e.Name = fmt.Sprintf("%s-etcd-%d", k.Name, i)
		b, err := etcdConfigToBytes(e, extraArgs)
		if err != nil {
			return nil, err
		}
		name := fmt.Sprintf("config.%s-etcd-%d", k.Name, i)
		data[name] = b
	}
...
func (k *KubernetesConfig) dirFromBytes(base string, data map[string][]byte) error {
	epoch := time.Unix(0, 0)
