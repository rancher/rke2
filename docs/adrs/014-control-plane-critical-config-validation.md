...
Architecture in K3s:
* Add a hidden CLI flag in K3s `critical-extra-cp-config` that fills a new variable `ServerConfig.CriticalExtraConfig`
* `CriticalExtraConfig` be part of the type CriticalControlArgs struct
* If the remote server is down-level and did not include the variable or fill it with any value, use the local value to allow for temporary mismatch during upgrades.

Architecture in RKE2:
* Add the hidden CLI flag in K3s `critical-extra-cp-config` as "dropFlag"
* Marshall cni, ingress-controller and prime variables as part of the critical-extra-cp-config flag (cmds.ServerConfig.CriticalExtraConfig, which is what gets propagated into K3s)
