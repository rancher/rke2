...
// E2E_RELEASE_VERSION=v1.23.1+rke2r2 (default: latest commit from master)
...
var _ = Describe("Verify Create", Ordered, func() {
	Context("Cluster :", func() {
		...
		It("save s3 snapshot using CLI", func() {
			res, err := tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot save " +
		...
		It("creates s3 config secret", func() {
			res, err := e2e.CreateKubeSecretSnapshots(tc.KubeconfigFile, true)
			Expect(err).NotTo(HaveOccurred())
			Expect(res).To(ContainSubstring("secret/rke2-etcd-s3-config created"))
		})
		...
		It("lists saved s3 snapshot", func() {
			res, err := tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot list")
			Expect(err).NotTo(HaveOccurred())
			Expect(res).To(ContainSubstring("file:///var/lib/rancher/rke2/server/db/snapshots/on-demand-server-0"))
			Expect(res).To(ContainSubstring("s3://test-bucket/test-folder/on-demand-server-0"))
		})
		It("save 3 more s3 snapshots", func() {
			for _, i := range []string{"1", "2", "3"} {
				res, err := tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot save --name special-" + i)
				Expect(err).NotTo(HaveOccurred())
		...
		It("delete first on-demand s3 snapshot", func() {
			_, err := tc.Servers[0].RunCmdOnNode("sudo rke2 etcd-snapshot ls >> ./snapshotname.txt")
			Expect(err).NotTo(HaveOccurred())
			snapshotName, err := tc.Servers[0].RunCmdOnNode("grep -Eo 'on-demand-server-0-([0-9]+)' ./snapshotname.txt | head -1")
			Expect(err).NotTo(HaveOccurred())
			res, err := tc.Servers[0].RunCmdOnNode("sudo rke2 etcd-snapshot delete " + snapshotName)
			Expect(err).NotTo(HaveOccurred())
			Expect(res).To(ContainSubstring("Snapshot " + strings.TrimSpace(snapshotName) + " deleted"))
		})
		It("prunes s3 snapshots", func() {
			_, err := tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot save")
			Expect(err).NotTo(HaveOccurred())
			time.Sleep(time.Second)
			_, err = tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot save")
			Expect(err).NotTo(HaveOccurred())
			time.Sleep(time.Second)
			_, err = tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot prune")
			Expect(err).NotTo(HaveOccurred())
			// There should now be 4 on-demand snapshots - 2 local, and 1 on s3
			res, err := tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot ls 2>/dev/null | grep on-demand | wc -l")
			Expect(err).NotTo(HaveOccurred())
		...
		It("ensure snapshots retention is working in s3 and local", func() {
			...
			res, err := tc.Servers[0].RunCmdOnNode("rke2 etcd-snapshot ls 2>/dev/null | grep etcd-snapshot | wc -l")
			Expect(err).NotTo(HaveOccurred())
			Expect(strings.TrimSpace(res)).To(Equal("3"))
		})
	})
