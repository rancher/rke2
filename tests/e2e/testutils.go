...
func scpWindowsRKE2Artifacts(nodeNames []string) error {
	...
	for _, node := range nodeNames {
		...
		if _, err := RunCmdOnWindowsNode(`mv C:\tmp\sha256sum-windows-amd64.txt C:\tmp\sha256sum-amd64.txt`, node); err != nil {
			return err
		}
		...
		for _, image := range images {
			cmd := fmt.Sprintf("mv C:\\tmp\\%s C:\\var\\lib\\rancher\\rke2\\agent\\images\\", filepath.Base(image))
			if _, err := RunCmdOnNode(cmd, node); err != nil {
				return err
			}
		}
