...
// Node Struct to parse node output
type Node struct {
	Name       string
	Status     string
	Roles      string
...
// Pod Struct to parse pod output
type Pod struct {
	NameSpace string
	Name      string
	Ready     string
...
// ConfigureSSH toeatablish ssh connection to node
...
// RunCmdOnNode Runs command passed from within the node ServerIP
...
// FetchNodeExternalIP Fetch external ip from the node
...
// FetchIngressIP used in ingress tests
