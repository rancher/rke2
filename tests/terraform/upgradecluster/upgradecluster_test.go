...
var _ = Describe("Upgrade Tests:", func() {
			...
			Eventually(func(g Gomega) {
				...
				for _, node := range nodes {
					g.Expect(node.Status).Should(Equal("Ready"),
						"Nodes should all be in Ready state", node.Name)
				}
			...
			Eventually(func(g Gomega) {
				...
				for _, pod := range pods {
					if strings.Contains(pod.Name, "helm-install") {
						...
						g.Expect(numRunning[0]).Should(Equal(numRunning[1]), pod.Name,
	...
	Context("Preupgrade Validations:", func() {
		It("Verifies ClusterIP Service Preupgrade", func() {
			...
			Eventually(func(g Gomega) {
				cmd := "kubectl get pods -n " + namespace + " -o=name " +
					"-l k8s-app=nginx-app-clusterip --field-selector=status.phase=Running " +
					"--kubeconfig=" + createcluster.KubeConfigFile
				res, err := terraform.RunCommand(cmd)
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(res).Should(ContainSubstring("test-clusterip"))
			}, "240s", "5s").Should(Succeed())

			clusterip, port, _ := terraform.FetchClusterIP(createcluster.KubeConfigFile,
				namespace, "nginx-clusterip-svc")
		...
		It("Verifies NodePort Service Preupgrade", func() {
			...
			nodeExternalIP := terraform.FetchNodeExternalIP(createcluster.KubeConfigFile)
			cmd := "kubectl get service -n " + namespace + " nginx-nodeport-svc --kubeconfig=" +
				createcluster.KubeConfigFile + " --output jsonpath=\"{.spec.ports[0].nodePort}\""
			...
			for _, ip := range nodeExternalIP {
				Eventually(func(g Gomega) {
					cmd := "kubectl get pods -n " + namespace + " -o=name -l k8s-app=nginx-app-nodeport " +
		...
		It("Verifies Ingress Preupgrade", func() {
			...
			Eventually(func(g Gomega) {
				cmd := "kubectl get pods -n " + namespace + " -o=name -l k8s-app=nginx-app-ingress " +
					"--field-selector=status.phase=Running --kubeconfig=" + createcluster.KubeConfigFile
				res, err := terraform.RunCommand(cmd)
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(len(ingressIps)).To(Equal(len(nodes)),
		...
		It("Verifies DNS Access Preupgrade", func() {
			...
			Eventually(func(g Gomega) {
				cmd := "kubectl get pods dnsutils " + "-n " + namespace +
					" --kubeconfig=" + createcluster.KubeConfigFile
			...
			Eventually(func(g Gomega) {
				cmd := "kubectl -n " + namespace + " --kubeconfig=" +
					createcluster.KubeConfigFile + " exec -t dnsutils -- nslookup kubernetes.default"
				res, _ := terraform.RunCommand(cmd)
	...
	Context("Upgrade via SUC:", func() {
		It("Verifies Upgrade", func() {
			...
			Eventually(func(g Gomega) {
				...
				for _, node := range nodes {
					g.Expect(node.Status).Should(Equal("Ready"),
						"Nodes should all be in Ready state after upgrading", node.Name)
					g.Expect(node.Version).Should(Equal(k8sVersion),
	...
	Context("Postupgrade Validations:", func() {
		It("Verifies ClusterIP Service Postupgrade", func() {
			...
			cmd := "kubectl get pods -n " + namespace + " -o=name -l " +
				"k8s-app=nginx-app-clusterip --field-selector=status.phase=Running --kubeconfig=" +
				createcluster.KubeConfigFile
			res, err := terraform.RunCommand(cmd)
		...
		It("Verifies NodePort Service Postupgrade", func() {
			...
			for _, ip := range nodeExternalIP {
				Eventually(func(g Gomega) {
					cmd := "kubectl get pods -n " + namespace + " -o=name -l " +
						"k8s-app=nginx-app-nodeport --field-selector=status.phase=Running --kubeconfig=" +
						createcluster.KubeConfigFile
					res, err := terraform.RunCommand(cmd)
		...
		It("Verifies Daemonset Postupgrade", func() {
			...
			Expect(count).Should((Equal(len(nodes))),
				"Daemonset pod count does not match node count")
		})
